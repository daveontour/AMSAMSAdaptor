    ...
    public class ModelFlight
    {
        ...
        public FlightId GetFlightId()
        {
            try
            {
                ...
                if (FlightProperties.ContainsKey("AirportIATA"))
                {
                    ...
                    {
                        codeContextField = CodeContext.IATA,
                        valueField = FlightProperties["AirportIATA"].Value
                    };
                ...
                {
                    codeContextField = CodeContext.IATA,
                    valueField = FlightProperties["AirlineIATA"].Value
                };
                LookupCode[] al = { alCode };

                FlightId flightID = new FlightId
                {
                    flightKindField = FlightProperties["Nature"].Value == "Arrival" ? FlightKind.Arrival : FlightKind.Departure,
                    airportCodeField = ap,
                    airlineDesignatorField = al,
                    scheduledDateField = Convert.ToDateTime(FlightProperties["ScheduledDate"].Value),
                    flightNumberField = FlightProperties["FlightNumber"].Value
                };
        ...
        public FlightId GetLinkedFlightId()
        {
            try
            {
                ...
                {
                    codeContextField = CodeContext.IATA,
                    valueField = FlightProperties["LinkedAirportIATA"].Value
                };
                ...
                {
                    codeContextField = CodeContext.IATA,
                    valueField = FlightProperties["LinkedAirlineIATA"].Value
                };
                LookupCode[] al = { alCode };

                FlightId flightID = new FlightId
                {
                    flightKindField = FlightProperties["LinkedNature"].Value == "Arrival" ? FlightKind.Arrival : FlightKind.Departure,
                    airportCodeField = ap,
                    airlineDesignatorField = al,
                    scheduledDateField = Convert.ToDateTime(FlightProperties["LinkedScheduledDate"].Value),
                    flightNumberField = FlightProperties["LinkedFlightNumber"].Value
                };
        ...
        public FlightUpdateInformation GetFlightUpdateInformation()
        {
            ...
            {
                updateField = GetPropertValues(),
                tableValueUpdateField = GetTableValue(),
                activityUpdateField = GetActivityUpdate(),
                eventUpdateField = GetEventUpdate()
            };
        ...
        public TableValueUpdate[] GetTableValue()
        {
            ...
            foreach (XmlNode table in node.SelectNodes("//amsx-datatypes:TableValue", nsmgr))
            {
                ...
                {
                    propertyNameField = table.Attributes["propertyName"]?.Value
                };
                TableRow[] rows = { };
                ...
                foreach (XmlNode row in table.SelectNodes(".//amsx-datatypes:Row", nsmgr))
                {
                    ...
                    foreach (XmlNode field in row.SelectNodes(".//amsx-datatypes:Value", nsmgr))
                    {
                        PropertyValue pv = new PropertyValue
                        {
                            propertyNameField = field.Attributes["propertyName"]?.Value,
                            valueField = field.InnerText
                        };
        ...
        public EventUpdate[] GetEventUpdate()
        {
            ...
            foreach (XmlNode e in node.SelectNodes("//amsx-datatypes:Event", nsmgr))
            {
                ...
                {
                    codeField = e.Attributes["code"]?.Value
                };
                PropertyValue[] valueField = { };
                foreach (XmlNode v in e.SelectNodes(".///amsx-datatypes:Value"))
                {
                    PropertyValue pv = new PropertyValue
                    {
                        propertyNameField = v.Attributes["propertyName"]?.Value,
                        valueField = v.InnerText
                    };
                    valueField = valueField.Append(pv).ToArray();
                }
